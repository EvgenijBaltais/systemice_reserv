    ...
    $('#main-form_submit').on('click', function(e){
        ...
        var nameVal = name.val(),
            ...
            if (nameVal !='' && phoneVal != '') {
                ...
                var formData = form.serialize() + '&knight_id=11398&hotelname=' + hotelname;
                ...
                $.ajax({
                    ...
                    /* Добавление цели на отправку формы счетчика яндекс метрики */
                        yaCounter44929225.reachGoal('get-price');
                    /* Добавление цели на отправку формы счетчика яндекс метрики */
...
(function($) {
    ...
    $(document).ready(function() {
        ...
        window.tildaForm.cloudpaymentPay = function($jform, arArgs) {
            ...
            if (arArgs.cloudpayments && arArgs.cloudpayments.recurrent) {
            ...
            window.cloudpaymentshandler.charge({
                ...
                invoiceId: arArgs.invoiceId,
                email: arArgs.accountId,
                data: {
                    projectid: arArgs.projectid
                }
            }, function(options) {
                $jpopup.css({
                    'z-index': $jpopup.data('old-z-index')
                });
                var virtPage = '/tilda/' + $jform.attr('id') + '/payment/';
                var virtTitle = 'Pay order in form ' + $jform.attr('id');
                var virtPrice = arArgs.amount;
